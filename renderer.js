...
const versionCheckCooldown = 60000; // 1分钟冷却时间
  ...
  dialog.innerHTML = `
    ...
    <div class="modern-modal-body">
      <div style="margin-bottom: 20px;">
        <h4 style="font-size: 14px; font-weight: 600; color: #1d1d1f; margin-bottom: 12px;">支持的导入格式：</h4>
        
        <div style="background: #f0f9ff; border: 1px solid #bae6fd; border-radius: 8px; padding: 12px; margin-bottom: 12px;">
          <div style="font-size: 13px; font-weight: 600; color: #0369a1; margin-bottom: 6px;">📄 CSV 格式 (.csv)</div>
          <div style="font-size: 12px; color: #0c4a6e; line-height: 1.6;">
            第一行为表头：email,password,name（name可选）<br>
            后续每行一个账号，用逗号分隔
          </div>
          <pre style="background: #ffffff; border: 1px solid #e0f2fe; border-radius: 4px; padding: 8px; margin: 8px 0 0 0; font-size: 11px; color: #1d1d1f; overflow-x: auto; font-family: 'Monaco', 'Menlo', monospace;">email,password,name
user1@example.com,pass123,用户1
user2@example.com,pass456,用户2</pre>
        </div>
        
        <div style="background: #f5f5f7; border: 1px solid #e5e5ea; border-radius: 8px; padding: 12px;">
          <div style="font-size: 13px; font-weight: 600; color: #6e6e73; margin-bottom: 6px;">📋 JSON 格式 (.json)</div>
          <div style="font-size: 12px; color: #86868b; line-height: 1.6;">
            JSON 数组，每个对象包含 email、password 字段
          </div>
          <pre style="background: #ffffff; border: 1px solid #e5e5ea; border-radius: 4px; padding: 8px; margin: 8px 0 0 0; font-size: 11px; color: #1d1d1f; overflow-x: auto; font-family: 'Monaco', 'Menlo', monospace;">[{"email": "user@example.com", "password": "pass123"}]</pre>
        </div>
      </div>
      
      <div class="form-tip" style="background: #e3f2fd; border-color: #90caf9; color: #1976d2;">
        <i data-lucide="info" style="width: 16px; height: 16px; flex-shrink: 0;"></i>
        <span>点击确定后将打开文件选择器，请选择要导入的 CSV 或 JSON 文件</span>
      </div>
    </div>
...
function selectImportFile() {
  ...
  fileInput.onchange = async (event) => {
    ...
    if (!fileName.endsWith('.json') && !fileName.endsWith('.csv')) {
      showCustomAlert('请选择 JSON 或 CSV 格式文件', 'error');
      return;
    }
...
async function processImportFile(content, filename = 'unknown') {
  ...
  if (!content || content.trim() === '') {
    showCustomAlert('文件内容为空，请选择有效的文件！', 'error');
    return;
  }
  ...
  if (isCSV) {
    // 解析 CSV 格式
    console.log('📄 检测到 CSV 格式，开始解析...');
    const lines = cleanContent.split(/\r?\n/).filter(line => line.trim());
    
    if (lines.length === 0) {
      showCustomAlert('CSV 文件内容为空！', 'error');
      return;
    }
    ...
    // 查找列索引
    const emailIndex = headers.findIndex(h => h === 'email' || h === '邮箱');
    const passwordIndex = headers.findIndex(h => h === 'password' || h === '密码');
    const nameIndex = headers.findIndex(h => h === 'name' || h === '名称' || h === '备注');
    
    // 如果第一行看起来像数据而不是表头（包含@符号），则当作数据处理
    const firstLineIsData = lines[0].includes('@');
    const startLine = firstLineIsData ? 0 : 1;
    
    // 如果没有找到表头，假设顺序为 email,password,name
    const useDefaultOrder = emailIndex === -1 || passwordIndex === -1;
    
    if (useDefaultOrder && !firstLineIsData) {
      console.log('⚠️ 未找到标准表头，假设列顺序为: email, password, name');
    }
    ...
    for (let i = startLine; i < lines.length; i++) {
      ...
      if (!password) {
        errors.push(`第 ${i + 1} 行缺少密码`);
        continue;
      }

      accounts.push({ email, password, note: name || '' });
    }
    
    console.log(`✅ CSV 解析完成，有效账号: ${accounts.length}，错误: ${errors.length}`);
    
  } else {
    // 解析 JSON 格式
    console.log('📋 检测到 JSON 格式，开始解析...');
    let parsed;
    ...
    parsed.forEach((item, index) => {
      ...
      const note = item.name || item.note || '';
      ...
      if (!password) {
        errors.push(`第 ${index + 1} 条记录缺少密码`);
        return;
      }

      accounts.push({ email, password, apiKey, note });
    });
  }
